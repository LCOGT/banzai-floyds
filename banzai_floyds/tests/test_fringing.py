...
from banzai_floyds.orders import find_fringe_offset, fit_smooth_fringe_spline, FringeMaker, FringeCorrector
...
def test_find_fringe_offset():
    ...
    fringe_data = np.sin(frame.input_fringe_wave_number * frame.wavelengths.data[red_order])
    fringe_data = 1.0 + 0.5 * (x2d / np.max(x2d)) * fringe_data
    # fit the fringe offset
    fringe_spline = fit_smooth_fringe_spline(frame)
...
def test_create_super_fringe():
    ...
    np.testing.assert_allclose(frame.data[in_order], frames[0].input_fringe_frame[in_order])
...
def test_correct_fringe():
    ...
    in_order = frame.orders.data == 1
    np.testing.assert_allclose(original_data[in_order] / frame.input_fringe_frame[in_order],
                               output_frame.data)
