...
def fit_profile_width(data, profile_fits, poly_order=3, default_width=8.0):
    ...
    profile_widths = [Legendre.fit(order_data['wavelength'], order_data['width'], deg=poly_order)
                      for order_data in profile_width_table.group_by('order').groups]
...
def fit_profile_centers(data, polynomial_order=5, profile_width=4):
    ...
    for data_to_fit in data.groups:
        ...
        snrs = [matched_filter_metric([center,], data_to_fit['data'], data_to_fit['uncertainty'],
