...
def correct_frame(path: str) -> tuple:
    ...
    try:
        ...
        measured = continuum > 0
        pattern = np.ones_like(image.data)
        pattern[measured] = raw_data[measured] / continuum[measured]
        record['pattern_before'], record['extent'] = fringe_region_cutout(image, pattern, cutoff)
        fractional_noise = np.zeros_like(image.data)
        fractional_noise[measured] = image.uncertainty[measured] / continuum[measured]
        record['noise'], _ = fringe_region_cutout(image, fractional_noise, cutoff)
        pattern = np.ones_like(image.data)
        pattern[measured] = image.data[measured] / continuum[measured]
        ...
        noise = np.nanmedian(record['noise'][usable])
